...
const SelectTipoCliente = ({ openClose, setOpenClose, setTipoClienteSelected, tipoClienteSelected }) => {
    const { verificarToken } = useContext(ContextGlobal);
    const handleClickMotivoVisita = (tipo) => {
        verificarToken();
        setTipoClienteSelected(tipo);
        setOpenClose(false);
    }
    return (
        <ModalAlert ocultarInfoGe={true} funcao={() => setOpenClose(false)} openModal={openClose}>
            <View style={styles.appBar}>
                ...
                <Text style={styles.textAppBar}>SELECIONE O TIPO DO CLIENTE</Text>
            </View>
            <AreaModalBtnInput>
                <ButtonInput borderBottomWidth={true} funcao={() => handleClickMotivoVisita(0)} texto='padrão' >
                ...
                <ButtonInput funcao={() => handleClickMotivoVisita(1)} texto='generico' >
